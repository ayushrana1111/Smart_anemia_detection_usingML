...
def generate_pdf(name, age, category, weight, hemoglobin, mch, mchc, mcv,
                 severity, anemia_type, ml_result, type_advice, prevention):
    pdf = FPDF()
    pdf.add_page()
    ...
    pdf.set_xy(0, 8)
    pdf.cell(210, 10, "Smart Anemia Detection System", align="C", ln=True)
    pdf.set_font("Helvetica", "", 11)
    pdf.cell(210, 8, "Patient Diagnosis Report", align="C", ln=True)
    ...
    pdf.set_font("Helvetica", "B", 13)
    pdf.set_text_color(220, 53, 69)
    pdf.cell(0, 8, "Blood Test Parameters", ln=True)
    pdf.set_draw_color(220, 53, 69)
    pdf.line(15, pdf.get_y(), 195, pdf.get_y())
    pdf.ln(3)

    params = [
        ("Hemoglobin (Hb)", f"{hemoglobin} g/dL", "Male >= 13, Female >= 12"),
    ...
    pdf.cell(50, 8, "Your Value", border=1, fill=True)
    pdf.cell(65, 8, "Normal Range", border=1, fill=True, ln=True)
    pdf.set_font("Helvetica", "", 10)
    for param, value, normal in params:
        pdf.set_x(15)
        pdf.cell(70, 8, param, border=1)
        pdf.cell(50, 8, value, border=1)
        pdf.cell(65, 8, normal, border=1, ln=True)
    ...
    pdf.set_font("Helvetica", "B", 13)
    pdf.set_text_color(220, 53, 69)
    pdf.cell(0, 8, "Diagnosis Results", ln=True)
    pdf.set_draw_color(220, 53, 69)
    pdf.line(15, pdf.get_y(), 195, pdf.get_y())
    pdf.ln(3)
    ...
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("Helvetica", "", 10)
    all_advice = type_advice + "\n" + prevention
    for line in all_advice.split("\n"):
    ...
    pdf.cell(0, 10, "Generated by Smart Anemia Detection System | Group 22 | B.Tech CSE 2026", align="C")
    ...
    def diagnosis_page():
            ...
            if mcv < 80:
                if mchc < 32:
                    return "Iron Deficiency Anemia", (
                        ...
                        "Avoid tea/coffee after meals - they block iron absorption."
                    )
                else:
                    return "Thalassemia (Possible)", (
                        ...
                        "Do NOT self-supplement iron - it can be harmful in Thalassemia."
                    )
            elif mcv > 100:
                if mch > 32:
                    return "Vitamin B12 / Folate Deficiency Anemia", (
                        ...
                        "Avoid alcohol - it depletes B12 and Folate rapidly."
                    )
                else:
                    return "Macrocytic Anemia (Mixed cause)", (
                        ...
                        "Do not self-diagnose - get tested before supplementing."
                    )
            else:
                if mchc < 32:
                    return "Anemia of Chronic Disease", (
                        ...
                        "Monitor kidneys and inflammation markers with regular blood tests."
                    )
                else:
                    return "Hemolytic / Blood Loss Anemia (Possible)", (
                        ...
                        "This type requires urgent diagnosis - do not delay medical consultation."
                    )
        ...
        if st.button("🔍 Detect Anemia", use_container_width=True):
            ...
            **Patient:** {name} | **Age:** {age} | **Category:** {category} | **Weight:** {weight} kg
            ...
            # PDF Download Button
            st.markdown("---")
            pdf_path = generate_pdf(name, age, category, weight, hemoglobin,
                                    mch, mchc, mcv, severity, anemia_type,
                                    ml_result, type_advice, prevention)
            with open(pdf_path, "rb") as f:
                st.download_button(
                    label="📄 Download Diagnosis Report (PDF)",
                    data=f,
                    file_name=f"Anemia_Report_{name}.pdf",
